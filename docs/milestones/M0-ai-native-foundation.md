...
3. **Branch protection — attempted, blocked.** Private free-plan repo: neither
   classic protection nor rulesets are available (HTTP 403). Documented in
   ADR 0004; enable by making the repo public or upgrading to Pro.
...
## Open CI findings (for human review)

- **CodeQL** runs but the upload fails: *"Code scanning is not enabled for this
  repository."* Code scanning needs a public repo or Advanced Security. The
  `codeql.yml` workflow is correct and will pass once code scanning is
  available; until then it reports failure.
- **Gitleaks** passes on `main` but **fails on Dependabot PRs** —
  `gitleaks/gitleaks-action@v2` throws inside its bundled `index.js` on
  Dependabot-triggered `pull_request` events (known action issue). The `main`
  branch / normal PR scan works.
- Both are tracked as follow-ups; neither blocks the foundation itself.
