"use client"

// Route error boundary for /reviews (task #116). Covers ONLY unexpected
// render-time failures of the chooser route — expected review failures (not
// imported, missing API key, GitHub failure, LLM failure) are in-page error
// states handled inside the review flow, not here.

import { AppNav, IconAlert } from "./_components/chrome"

export default function ReviewsError({ reset }: { reset: () => void }) {
  return (
    <div className="screen">
      <AppNav active="reviews" />
      <main className="page">
        <div className="container-narrow">
          <header>
            <div className="page-eyebrow">
              <span className="dot" /> Diff Review · M8
            </div>
            <h1 className="page-title">Diff reviews</h1>
          </header>

          <section className="status-region">
            <div className="status-card" data-error="true">
              <div className="status-head">
                <div className="status-icon error" aria-hidden="true">
                  <IconAlert size={18} />
                </div>
                <h2 className="status-title">Something went wrong</h2>
              </div>
              <p className="status-body">
                The reviews page hit an unexpected error. This is usually
                temporary — try reloading.
              </p>
              <div className="status-actions">
                <button
                  type="button"
                  className="btn btn-primary"
                  onClick={reset}
                >
                  Try again
                </button>
              </div>
            </div>
          </section>
        </div>
      </main>
    </div>
  )
}
